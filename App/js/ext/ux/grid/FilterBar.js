 ...
 * @updated byt Ing. Peter Skultety - compatibility with Ext5
*/
...
Ext.define('Ext.ux.grid.FilterBar', {
    ...
    showHideButtonTooltipDo: 'Показать фильтры',    // button tooltip show
    showHideButtonTooltipUndo: 'Спрятать фильтры',    // button tooltip hide
    ...
    clearAllButtonTooltip: 'Очистить все фильтры',  // button tooltip
    ...
    unsetup: function (grid) {
        ...
        /*Закомментировал после перехода на тему neptune
    ...
    parseInitialFilters: function () {
        ...
        me.filterArray = [];
        if (me.grid.each && me.grid.store && me.grid.store.filters && me.grid.store.filters.each) {
    ...
    applyFilters: function (field) {
        ...
        if (me.filterArray.length > 0) {
            ...
            if (grid.store.filters) grid.store.filters.clear();
            // MIGRATION start
